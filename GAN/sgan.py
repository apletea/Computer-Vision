...
#        model.add(BatchNormalization(name='batch3'))
    ...
    def build_discriminator(self):
        ...
        model.add(Conv2D(32, kernel_size=3, strides=2, input_shape=(self.img_cols, self.img_rows, 1), padding="same"))
        model.add(Activation("relu"))
...
#        model.add(BatchNormalization(name='batch5'))
        ...
        label = Dense(self.num_classes+1)(features)
        label = Lambda(lambda x: tf.nn.softmax(x))(label)
    ...
    def train(self, epochs, batch_size=128, sample_interval=50):
        ...
        for epoch in range(epochs):
            ...
            #  Train Generator
            # ---------------------
            plot_model(self.combined, to_file='combined.png')
    ...
    def sample_images(self, epoch):
        ...
        for i in range(r):
            for j in range(c):
                ...
                axs[i,j].axis('off')
                cnt += 1
        fig.savefig("images_1/mnist_%d.png" % epoch)
        plt.close()
